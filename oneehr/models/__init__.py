...
Merges the old registry.py, constants.py, and utils.py into a single entry point.
"""
...
def _gru_kwargs(mc, input_dim, out_dim):
    kw = _standard_kwargs(mc, input_dim, out_dim)
    kw["cell"] = "gru"
    return kw


def _lstm_kwargs(mc, input_dim, out_dim):
    kw = _standard_kwargs(mc, input_dim, out_dim)
    kw["cell"] = "lstm"
    return kw
...
_SPECS: dict[str, ModelSpec] = {
    "gru": ModelSpec("oneehr.models.recurrent", "RecurrentModel", "RecurrentTimeModel", "gru", _gru_kwargs),
    "lstm": ModelSpec("oneehr.models.recurrent", "RecurrentModel", "RecurrentTimeModel", "lstm", _lstm_kwargs),
    "transformer": ModelSpec("oneehr.models.transformer", "TransformerModel", "TransformerTimeModel", "transformer", _transformer_kwargs),
    "tcn": ModelSpec("oneehr.models.tcn", "TCNPatientModel", "TCNTimeModel", "tcn", _tcn_kwargs),
}


def register(name: str, spec: ModelSpec) -> None:
    """Register a new model spec (plugin-friendly)."""
    _SPECS[name] = spec
