"""GraphCare-style knowledge graph enhanced EHR models."""
...
class GraphCareBackbone(nn.Module):
    def __init__(
        self,
        num_groups: int,
        hidden_dim: int = 128,
    ...
    def forward(
        ...
        *,
        ...
        if group_values is None or group_mask is None:
            raise ValueError("GraphCare requires group_values and group_mask in forward()")
...
class GraphCareModel(nn.Module):
    def __init__(
        self,
        input_dim: int,
        hidden_dim: int = 128,
    ...
    def forward(
        ...
        *,
        ...
        seq, summary = self.backbone(
            lengths,
            group_values=group_values,
            group_mask=group_mask,
            visit_time=visit_time,
        )
        patient = torch.cat([last_by_lengths(seq, lengths), summary], dim=-1)
        return self.head(patient)


class GraphCareTimeModel(nn.Module):
    def __init__(self, **kwargs):
        super().__init__()
        self.core = GraphCareModel(**kwargs)
