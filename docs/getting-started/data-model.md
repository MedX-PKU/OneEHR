...
!!! tip
    Models with a static branch (MCGRU, DrAgent) can leverage static features as a separate input. All other models receive static features concatenated with dynamic features.
...
### When to use `label.csv` vs `labels.fn`

- Use `label.csv` when labels are pre-computed and stored alongside your dataset
- Use `labels.fn` (a Python function) when labels should be derived dynamically from events -- this is recommended for rapid iteration

See [Label Functions](../guide/label-functions.md) for details on writing a `label_fn`.
...
```
dynamic.csv ──┐
              ├──→ preprocess ──→ binned.parquet ──→ views/*.parquet ──→ train
static.csv  ──┤                                                          │
              │                                                          ▼
label.csv ────┘   (or labels.fn)                                    models + metrics
```

1. **Preprocess** bins `dynamic.csv` into fixed time windows and builds feature columns
2. Static features from `static.csv` are encoded and optionally joined
3. Labels come from `label.csv` or are generated by `labels.fn`
4. Tabular views (patient or time) are materialized for modeling
5. All downstream commands (train, test, analyze) read from the materialized artifacts
