...
def test_statistical_tests_compute():
    ...
    preds = pd.concat(
        [
            pd.DataFrame({"patient_id": pids, "system": "model_a", "y_true": y_true, "y_pred": np.clip(y_true + rng.normal(0, 0.3, n), 0, 1)}),
            pd.DataFrame({"patient_id": pids, "system": "model_b", "y_true": y_true, "y_pred": np.clip(y_true + rng.normal(0, 0.4, n), 0, 1)}),
        ],
        ignore_index=True,
    )
